...
import { Loader2, User, CheckCircle, ArrowRight, ArrowLeft, PartyPopper } from "lucide-react";
...
const PublicBookingForm = () => {
  ...
  useEffect(() => {
    const fetchProfessionals = async () => {
      ...
      const { data, error } = await supabase
        ...
        .select('is_available, professionals!inner(id, full_name)')
      ...
      if (error) {
        ...
        const uniqueProfessionals = data.reduce<Professional[]>((acc, current) => {
          ...
          if (professional && !acc.find(item => item.id === professional.id)) {
            acc.push(professional as Professional);
          }
          return acc;
        }, []);
    ...
    const fetchAvailability = async () => {
      ...
      try {
        const { data, error: functionError } = await supabase.functions.invoke('get-availability', {
          body: {
            professionalId: Number(watchedProfessionalId),
        ...
        if (functionError) {
          ...
          const errorMessage = parsedResponse.error || functionError.message;
          const debugInfo = parsedResponse.debug || "Nenhuma informação de depuração da função.";
          
          if (isActive) {
            showError(errorMessage);
            const debugText = typeof debugInfo === 'object' ? JSON.stringify(debugInfo, null, 2) : debugInfo;
            setDebugMessage(`Erro: ${errorMessage}\n\nInformações de Depuração:\n${debugText}`);
            setAvailableTimes([]);
          }
          return;
        }
        ...
        if (isActive) {
          const formattedTimes = (data.availableTimes || []).map((isoString: string) => {
            return format(new Date(isoString), 'HH:mm');
          });
          setAvailableTimes(formattedTimes);
          if (data.debug) {
            const message = typeof data.debug === 'object' ? JSON.stringify(data.debug, null, 2) : data.debug;
        ...
        if (isActive) {
          showError("Ocorreu um erro inesperado de comunicação.");
          setDebugMessage(`Erro inesperado: ${e.message}`);
        }
      } finally {
        if (isActive) {
  ...
  const onSubmit = async () => {
    ...
    try {
      ...
      const { error } = await supabase.functions.invoke('create-public-appointment', {
        body: {
          professional_id: values.professional_id,
          services: values.services,
          full_name: values.full_name,
          phone: values.phone,
          start_time_iso: startTimeAsISO,
        },
      });
      ...
      showSuccess("Agendamento confirmado! Verifique seu WhatsApp.");
      setStep(STEPS.length + 1); // Go to success screen
    } catch (e: any) {
  ...
  if (step > STEPS.length) {
    return (
      <Card className="backdrop-blur-md bg-white/5 border border-white/10 shadow-xl text-center p-8">
        ...
        <p className="text-gray-300 mt-2">Seu horário foi confirmado com sucesso. Enviamos os detalhes para o seu WhatsApp.</p>
        <Button onClick={() => { form.reset(); setStep(1); }} className="mt-6">Agendar Novamente</Button>
      </Card>
    );
  }
        ...
        <div className="mt-6">
                  ...
                  {loadingProfessionals ? (
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                      {professionals.map((prof) => (
                        <div key={prof.id} onClick={() => field.onChange(String(prof.id))} className={cn("relative cursor-pointer rounded-lg border-2 p-4 text-center transition-all duration-200", field.value === String(prof.id) ? "border-primary bg-primary/10" : "border-white/20 bg-white/5 hover:border-primary/50")}>
                          {field.value === String(prof.id) && <CheckCircle className="absolute top-2 right-2 h-5 w-5 text-primary" />}
                          <User className="mx-auto h-8 w-8 mb-2 text-gray-400" />
                          <h4 className="font-semibold text-white">{prof.full_name}</h4>
                        </div>
                      ))}
                    </div>
                  ) : (
                    <div className="text-center text-gray-400 py-8">
                      <p>Nenhum profissional com horários de trabalho definidos foi encontrado.</p>
                      <p className="text-sm mt-2">Por favor, configure os horários no painel de administração.</p>
                    </div>
                  )}
          ...
          {step === 2 && (
            <Controller
              ...
              render={({ field, fieldState }) => (
                <div className="space-y-2">
                  <ServiceSelection value={field.value as Service[]} onChange={field.onChange} />
                  {fieldState.error && <p className="text-sm font-medium text-destructive mt-2">{fieldState.error.message}</p>}
                </div>
              )}
            />
                  ...
                  <div className="grid grid-cols-3 sm:grid-cols-4 gap-3 max-h-64 overflow-y-auto pr-2">
                    {isFetchingTimes ? [...Array(12)].map((_, i) => <Skeleton key={i} className="h-10 w-full bg-white/10" />)
                      : availableTimes.length > 0 ? availableTimes.map((time) => (
                        <Button key={time} type="button" variant={field.value === time ? "default" : "outline"} onClick={() => field.onChange(time)} className={cn("bg-white/5 border-white/20 hover:bg-white/20", field.value === time && "bg-primary hover:bg-primary/90")}>
                          {time}
                        </Button>
                      ))
                      : <div className="col-span-3 sm:col-span-4 text-center text-sm text-gray-400 py-8">
                          {watchedDate ? "Nenhum horário vago para esta data." : "Escolha uma data primeiro."}
                          {debugMessage && (
                            <pre className="mt-4 text-xs text-left bg-gray-800 p-2 rounded-md whitespace-pre-wrap">
                              {debugMessage}
                            </pre>
                          )}
                        </div>
          ...
          <div className="flex justify-between items-center pt-8 mt-4 border-t border-white/10">
            <Button type="button" variant="outline" onClick={prevStep} disabled={step === 1} className="bg-transparent border-white/20 hover:bg-white/10">
              <ArrowLeft className="h-4 w-4 mr-2" />
              Voltar
            </Button>
            <Button type="button" onClick={nextStep} disabled={loadingSubmit || isFetchingTimes}>
              {loadingSubmit ? <Loader2 className="h-4 w-4 animate-spin" /> :
                step === STEPS.length ? "Confirmar Agendamento" :
                <>
                  Próximo
